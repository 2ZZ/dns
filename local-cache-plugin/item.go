...
func newItem(m *dns.Msg, now time.Time, d time.Duration) *item {
	...
	i.stored = now.UTC()
	i.queryCount = 0
	i.sourceIPs = make(map[string]int)
...
func (i *item) recordQuery(sourceIP string) {
	i.mu.Lock()
	i.queryCount++
	i.sourceIPs[sourceIP]++
	i.mu.Unlock()
}

func (i *item) getStats() (int, map[string]int) {
	i.mu.RLock()
	count := i.queryCount
	ips := make(map[string]int, len(i.sourceIPs))
	for ip, c := range i.sourceIPs {
		ips[ip] = c
	}
	i.mu.RUnlock()
	return count, ips
